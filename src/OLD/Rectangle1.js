...
// Function to calculate the rotated bounding box for a shape and return vectors
...
const getObjectSnappingEdges = (node) => {
  ...
  Object.values(boundingBoxVectors).forEach(({ start, end }) => {
    const projected = projectPointOntoVector(absPos, start, end);
    vertical.push({
      guide: projected.x,
      offset: absPos.x - projected.x,
      snap: "custom"
    });
    horizontal.push({
      guide: projected.y,
      offset: absPos.y - projected.y,
      snap: "custom"
    });
  });
